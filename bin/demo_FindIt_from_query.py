...
try:
    ...
    print 'Supply pubmed advanced query in quotes as the argument to this script.'
    sys.exit()

try:
    outdir = sys.argv[2]
except IndexError:
    outdir = os.path.join(basedir, query.replace(' ', '_'))
...
csvfilename = get_filename(os.path.join(basedir, query.split()[0]+'.csv'))
